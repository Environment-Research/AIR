...
# Load Mimi package version of RICE 2010.
using MimiRICE2010
...
function construct_rice_air(inputs::RICE_AIR_inputs)
    ...
    # Load RICE2010 parameters needed to build RICE+AIR.
    rice_params = load_rice_parameters()

    # Read in RICE+AIR Parameters and data.
    air_params = load_air_parameters(inputs.SSP_scenario)
    ...
    # Set first period population to RICE2010 baseline population values.
    rice_air_population[1,:] = rice_params[:population][1,:]
    rice_air_population[2:end,:] = (air_params[:population][1:(inputs.nsteps-1),:])./1000.0

    # Create an instance of RICE2010 to modify by adding in AIR components.
    m = getrice()
    ...
    set_param!(m, :air_baseline_emissions, :SO2₀, air_params[:SO2₀])
    set_param!(m, :air_baseline_emissions, :PM25₀, air_params[:PM25₀])
    set_param!(m, :air_baseline_emissions, :NOX₀, air_params[:NOX₀])
    ...
    set_param!(m, :air_radiativeforcing, :forcoth, air_params[:total_exogeous_rf])
    ...
    set_param!(m, :air_lifeyears, :SO2₀, air_params[:SO2₀])
    set_param!(m, :air_lifeyears, :PM25₀, air_params[:PM25₀])
    set_param!(m, :air_lifeyears, :NOX₀, air_params[:NOX₀])
